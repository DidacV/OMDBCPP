...
{
    int date, duration, ns1, ns2;
    string name, ageRating, genres;

    if(input >> quoted(name) >> date)
    {
        if (input >> quoted(ageRating) >> quoted(genres))
        {
            if (input >> duration >> ns1 >> ns2)
            {
                movie = Movie(name, date, ageRating, genres, duration, ns1, ns2);
            }
        }
    } else {
